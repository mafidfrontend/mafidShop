...
function NavCenter() {
    ...
    useEffect(() => {
        ...
        axios
            .get(`https://nt.softly.uz/api/front/products/${params.pathname}`)
            .then((res) => {
        ...
        <div>
            <div className="max-w-screen-xl mx-auto pt-6 flex flex-wrap justify-between items-center gap-4 px-4 md:px-12">
                ...
                <div className="flex gap-6">
                    ...
                    <div className="flex flex-col items-center text-sm text-gray-800 cursor-pointer" onClick={() => setShowCart(true)}>
                        <Image width={30} height={30} src={shop} alt="shop" />
                        <p
                            onClick={() => dispatch(addToCart(produktId!))}
                            className="text-gray-800 rounded-2xl transition shadow"
                        >
                            Savatga qo‘shish
                        </p>
                    </div>
